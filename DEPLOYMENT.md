...
默认启动 50 个协程消费股票队列，但通过信号量把实际网页并发限制为 12；失败重试
使用异步退避，不占用其他股票的执行机会。主任务每天 16:30 执行，批次结束后立即
补偿剩余网络失败股票，次日 15:30 再审计上一交易日；自动补偿达到上限
后停止并记录错误，不会无限消耗代理。
浏览器页面运行时异常会额外获得 2 轮补偿机会，并在新页面上下文中重新请求。
